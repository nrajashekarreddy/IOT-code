...
const char* ssid = "Rihusoft";
const char* pwd = "R1hu50ft";
...
void loop()
{
   ...
   temperature = (refvoltage * sensor * 100) / 1023; //converting degree to celcius
    ...
    if (request.indexOf("/Thermo=OFF") != -1)
    {
      ...
      //client.println("<br>");
      client.println("<br><br><h1>IOT: POWER MONITORING</h1><br><br>");
      //client.println("<h3>Probable LM35 analog reading: <h3>");
      //client.print(sensor); //analog reading of the sensor
      //client.print("<br>");
      
      client.print("<h3>Power Monitoring Status: </h3>");
      if(request.indexOf("/AlwaysOn") != -1)
      {
        client.print("<h3>Always On<h3>");
        client.print("(Power Values updated every 5 seconds.)");
        client.println("<h3> Present Power Value: <h3>");
        client.print(temp);
        client.print("<b> Watts.</b>");

        //save the data to mysql, with http client
      HTTPClient http;
      String url = "http://192.168.1.5/iotproject/power/add.php?temp="+String(temp); //access the php file to write data
      Serial.println(url);     
      http.begin(url);
     ...
     else if(value == HIGH)
     {
      ...
      else if(value == LOW)
      {
         ...
         // client.println("<br>"); 
          client.println("<h3>Last measured Power: <h3>"); //print the last value of temp
          client.print(temp);
      ...
      client.println("<h3>Check the Power records by following the below Button: <h3>");
  client.println("<p>http://192.168.1.5/iotproject/power/show.php</p>");
      //client.println("<a href=\"/Display_power\"\">http://192.168.1.5/iotproject/power/show.php</a><br />");
      //client.println("<!DOCTYPE html>\n<html>\n<head>\n<style>\n.button {\n    background-color: #4CAF50;\n    border: none;\n    color: white;\n    padding: 15px 32px;\n    text-align: center;\n    text-decoration: none;\n    display: inline-block;\n    font-size: 16px;\n    margin: 4px 2px;\n    cursor: pointer;\n}\n</style>\n</head>\n<body>\n\n\n\n\n<a href=\"http://192.168.1.5/iotproject/power/show.php\" class=\"button\">Power Status</a>\n\n\n</body>\n</html>\n");
